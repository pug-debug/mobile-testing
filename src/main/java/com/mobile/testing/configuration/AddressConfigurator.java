...
public class AddressConfigurator {
  ...
  private static final String KILL_NODE_COMMAND = "killall node";
  ...
  public static void startService(int port) {
    ...
    LOG.info("Appium server was started in port: " + port);
  }
  ...
  private static boolean isPortFree(int port) {
    ...
    try(ServerSocket ignored = new ServerSocket(port)) {
      ...
      isFree = false;
      LOG.warn(format("Specified port %s is occupied. Process will be terminated", port));
    }
    return isFree;
  }
